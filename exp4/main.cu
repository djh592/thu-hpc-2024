...
const int H = 10000, W = 10000; // 输出的高度和宽度
const int kernel_size = 3; // 卷积核大小
const int n = (W + kernel_size - 1) * (H + kernel_size - 1); // 输入大小
float *input, *output; // CPU 内存
float *dev_input, *dev_output; // GPU 内存
float *answer = nullptr; // 正确答案
float *produce = nullptr; // 产生的答案
